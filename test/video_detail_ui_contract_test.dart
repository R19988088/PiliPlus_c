...
void main() {
  ...
  test('播放器全屏控制条和隐藏进度条使用统一 120px 左右空白', () {
    ...
    expect(
      playerView,
      contains('static const _fullScreenHiddenProgressBottom = 1.0;'),
    );
    expect(
      playerView,
  ...
  test('播放器支持设置全屏视频圆角裁切', () {
    ...
    expect(playerView, contains('EdgeInsets.only(bottom: _fullScreenVideoVerticalGap)'));
  ...
  test('非全屏非宽屏视频增加播放器高度', () {
    ...
    expect(videoPageView, isNot(contains('videoAspectRatio: () => _videoAspectRatio')));
    expect(videoPageView, contains('double? get _videoAspectRatio'));
    expect(videoPageView, isNot(contains('_displayAspectRatio')));
    ...
    expect(videoPageView, isNot(contains('videoDetailController.firstVideo.sar')));
